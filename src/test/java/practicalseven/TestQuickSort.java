...
import practicalseven.sort.QuickSort;

import java.util.Arrays;
import java.util.Random;

/**
 * A JUnit test suite for the QuickSort.
 *
...
public class TestQuickSort {
  ...
  @Test
  public void testQuickSortCorrectName() {
    assertEquals((new QuickSort()).getName(), "QuickSort");
  }

  @Test
  public void testQuickSortWithChar() {
  ...
  @Test
  public void testQuickSortWithInt() {
  ...
  @Test
  public void testQuickSortWithManyOrderedInts() {
    int[] originalArray = new int[MAXIMUM_NUMBER_ITEMS];
    for (int i = 0; i < originalArray.length; i++) {
      originalArray[i] = i;
    }
    int[] sortedArray = (new QuickSort()).sort(originalArray);
  ...
  @Test
  public void testQuickSortWithManyOrderedChar() {
    char[] originalArray = new char[MAXIMUM_NUMBER_CHARS];
    for (int i = 0; i < originalArray.length; i++) {
      originalArray[i] = (char)(i + '0');
    }
    char[] sortedArray = (new QuickSort()).sort(originalArray);
    assertTrue(isSorted(originalArray));
    assertTrue(isSorted(sortedArray));
  ...
  @Test
  public void testQuickSortWithManyReversedInts() {
    int[] originalArray = new int[MAXIMUM_NUMBER_ITEMS];
    for (int i = 0; i < originalArray.length; i++) {
      originalArray[i] = i;
    }
    int[] reversedArray = reverse(originalArray);
    int[] sortedArray = (new QuickSort()).sort(reversedArray);
    assertTrue(isSorted(originalArray));
    assertTrue(isSorted(sortedArray));
    int[] originalClone = Arrays.copyOf(originalArray, originalArray.length);
    Arrays.sort(originalClone);
  ...
  @Test
  public void testQuickSortWithManyReversedChars() {
    char[] originalArray = new char[MAXIMUM_NUMBER_CHARS];
    for (int i = 0; i < originalArray.length; i++) {
      originalArray[i] = (char)(i + '0');
    }
    char[] reversedArray = reverse(originalArray);
    char[] sortedArray = (new QuickSort()).sort(reversedArray);
    assertTrue(isSorted(originalArray));
    assertTrue(isSorted(sortedArray));
    char[] originalClone = Arrays.copyOf(originalArray, originalArray.length);
    Arrays.sort(originalClone);
  ...
  @Test
  public void testQuickSortWithManyRandomInts() {
  ...
  @Test
  public void testQuickSortWithManyRandomChars() {
    Random random = new Random();
